...
def voice_over(key: str, script: str, languages: str = "en") -> bytes:
    ...
    for character, dialogue in segments:
        try:
            ...
            raise RuntimeError(f"Failed to generate audio for {character}: {str(e)}")
...
def extract_character_audio(script: str, character: str, api_key: str, language: str = "en") -> bytes:
    """
    ...
    segments = parse_script_segments(script)
    character_dialogues = [
        dialogue for char, dialogue in segments if char == character
    ]
